 ...
 * @param {Array|string} types - An array of requested object trap types
 * @returns {Array} - Array of traps available for the requested type
 */
function getTrapKeys(...types) {
    return Object.key(trapDefinitions).filter(function _trapIterationCallback(trap) {
        return this.includes(trapDefinitions[trap]);
    }, types);
